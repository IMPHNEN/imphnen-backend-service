use super::super::dto::UsersUpdateRequestDto;
use crate::users::domain::{UserEntity, UserService};
use axum::{
	Extension, Json, extract::Multipart, http::HeaderMap, response::IntoResponse,
};
use imphnen_entities::{ResponseSuccessDto, RolesDetailQueryDto};
use imphnen_libs::{AppState, ENV, hash_password};
use imphnen_storage::{
	FileType, MinioConfig, create_minio_service_from_config, decode_base64_file,
...
pub async fn upload_file(
	...
	let minio_service = create_minio_service_from_config(minio_config)
		.await
		.map_err(|e| {
	...
	if !file_type.allowed_types().contains(&content_type.as_str()) {
		return Err(AppError::BadRequestError(format!(
			"File type does not match content type '{content_type}'"
		)));
	}
	...
	let permanent_url = format!("{}/{}/{}", ENV.cdn_url, bucket_name, object_path);
