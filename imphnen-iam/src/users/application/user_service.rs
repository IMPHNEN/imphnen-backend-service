...
impl UserService for UserServiceImpl {
	...
	async fn update_password(
		...
		let is_valid = verify_password(&old_password, &user.password).map_err(|_| {
			AppError::BadRequestError("Password verification failed".into())
		})?;
		if !is_valid {
		...
		let new_hash = hash_password(&new_password).map_err(|_| {
