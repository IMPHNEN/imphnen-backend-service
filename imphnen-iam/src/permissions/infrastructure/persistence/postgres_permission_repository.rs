...
use imphnen_entities::seaorm::auth::permissions::{
	ActiveModel as PermissionsActiveModel, Column as PermissionsColumn,
	Entity as PermissionsEntity, Model as PermissionsModel,
};
...
impl PermissionRepository for PostgresPermissionRepository {
	async fn find_all(
		...
		let mut query =
			PermissionsEntity::find().filter(PermissionsColumn::IsDeleted.eq(false));
	...
	async fn create(&self, entity: PermissionEntity) -> Result<String, AppError> {
		...
		Ok(format!(
			"Success create permission with id: {}",
