...
use imphnen_entities::seaorm::auth::roles::{
	ActiveModel as RolesActiveModel, Column as RolesColumn, Entity as RolesEntity,
	Model as RolesModel,
};
...
use sea_orm::prelude::*;
use sea_orm::{ActiveValue, Order, PaginatorTrait, QueryOrder, QuerySelect};
use std::sync::Arc;
use uuid::Uuid;
...
impl RoleRepository for PostgresRoleRepository {
	async fn find_all(
		...
		let mut query = RolesEntity::find().filter(RolesColumn::DeletedAt.is_null());
	...
	async fn create(&self, entity: RoleEntity) -> Result<RoleEntity, AppError> {
		let permissions_json =
			serde_json::to_value(&entity.permissions).map_err(|e| {
				AppError::InternalServerError(format!(
					"Failed to serialize permissions: {e}"
				))
			})?;
	...
	async fn update(
		...
		if let Some(perms) = permissions {
			let permissions_json = serde_json::to_value(&perms).map_err(|e| {
				AppError::InternalServerError(format!(
					"Failed to serialize permissions: {e}"
				))
			})?;
