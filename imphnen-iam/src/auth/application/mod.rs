...
use crate::auth::domain::types::{
	...
	RefreshTokenInput, RegisterInput, ResendOtpInput, VerifyEmailInput,
};
use crate::roles::domain::RoleRepository;
use crate::users::domain::{UserEntity, UserRepository};
use async_trait::async_trait;
use imphnen_email::send_email;
use imphnen_entities::{RolesDetailQueryDto, users::UserProfileExtensionDto};
use imphnen_libs::{
	decode_access_token, decode_refresh_token, encode_access_token,
	encode_refresh_token, encode_reset_password_token, environment, hash_password,
	verify_password,
};
...
fn entity_to_user_detail(user: UserEntity) -> AuthUserDetail {
	AuthUserDetail {
		id: user.id,
		email: user.email,
		...
		role: user.role,
		profile_extension: user.profile_extension,
...
impl AuthService for AuthServiceImpl {
	async fn login(&self, payload: LoginInput) -> Result<LoginOutput, AppError> {
		let user = self
			...
			.await
			.map_err(|_| {
				AppError::AuthenticationError("Email or password not correct".into())
			})?;
		if !user.is_active {
			return Err(AppError::AuthenticationError(
				"Account not active, please verify your email".into(),
			));
		}
		let valid =
			verify_password(&payload.password, &user.password).map_err(|_| {
				AppError::InternalServerError("Password verification failed".into())
			})?;
		if !valid {
			return Err(AppError::AuthenticationError(
				"Email or password not correct".into(),
			));
		}
		let access_token = encode_access_token(payload.email.clone(), user.id.clone())
			.map_err(|_| {
			AppError::InternalServerError("Failed to generate access token".into())
		})?;
		let refresh_token = encode_refresh_token(payload.email.clone(), user.id.clone())
			.map_err(|_| {
				AppError::InternalServerError("Failed to generate refresh token".into())
			})?;
		Ok(LoginOutput {
			user: entity_to_user_detail(user),
			token: AuthTokens {
				access_token,
				refresh_token,
			},
	...
	async fn login_mentor(
		...
		if user.role.name != "Mentor" {
			return Err(AppError::ForbiddenError(
				"User does not have mentor privileges".into(),
			));
		}
		let access_token = encode_access_token(payload.email.clone(), user.id.clone())
			.map_err(|_| {
	...
	async fn refresh_token(
		...
		let email = decode_refresh_token(&payload.refresh_token)
			.map_err(|_| AppError::AuthenticationError("Invalid refresh token".into()))?
			.claims
			.sub;
		let user = self
			.user_repo
			...
			if let Ok(user) = user_repo.find_by_email(payload.email.clone()).await {
				match encode_reset_password_token(user.email.clone(), user.id.clone()) {
					Ok(token) => {
						...
						if let Err(e) =
							send_email(&payload.email, "Reset Password Request", &msg)
						{
	...
	async fn new_password(&self, payload: NewPasswordInput) -> Result<(), AppError> {
		let email = decode_access_token(&payload.token)
			.map_err(|_| AppError::BadRequestError("Invalid or missing token".into()))?
			.claims
			.sub;
		let user = self
			.user_repo
